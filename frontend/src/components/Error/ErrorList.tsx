...
const ErrorList = ({ selectedProject }: Props) => {
  ...
  return (
    <table className="box-border flex h-full w-full flex-col items-center justify-between">
      <thead className="box-border flex h-[5%] w-full items-center rounded-full bg-gradient-to-r from-[#98B9E8FF] to-[#BBB7EA] shadow-md">
        <tr className="mx-5 flex h-full w-full items-center justify-between">
          <td className="box-border flex h-full w-[7%] items-center justify-center font-medium text-white">
            해결여부
          </td>
          <td className="box-border flex h-full w-[30%] items-center justify-center font-medium text-white">
            에러 종류
          </td>
          <td className="box-border flex h-full w-[13%] items-center justify-center font-medium text-white">
            서비스
          </td>
          <td className="box-border flex h-full w-[13%] items-center justify-center font-medium text-white">
            프로젝트
          </td>
          <td className="box-border flex h-full w-[15%] items-center justify-center font-medium text-white">
            날짜
          </td>
      ...
      {/* BODY */}
      <tbody className="box-border h-[94%] w-full divide-y divide-gray-200 overflow-y-scroll scrollbar-webkit">
        {errorList.map((error) => (
          <tr
            key={error.errorId}
            className="mx-3 box-border flex h-12 w-full cursor-pointer items-center justify-between p-2 py-2 font-light text-gray-800 hover:bg-amber-100"
          ...
          >
            ...
            <td className="box-border flex h-full w-[13%] items-center justify-center whitespace-nowrap">
            ...
            <td className="box-border flex h-full w-[14%] items-center justify-center whitespace-nowrap">
              {error.time.split("T")[0]}
            </td>
            <td className="box-border flex h-full w-[11%] items-center justify-center whitespace-nowrap text-gray-400">
              {/* TODO 연결된 에러 노트 개수 가져오나요? */}1 Notes
            </td>
          </tr>
        ))}
      </tbody>
