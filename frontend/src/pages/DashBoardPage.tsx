import ErrorList from "../components/Error/ErrorList/ErrorList";

const DashBoardPage = () => {
  return (
    <div className="bg-default">
      <div className="flex h-full w-full items-center justify-center text-xl text-black">
        <div className="h-full w-full">
          <div className="flex h-[55%] w-full rounded-xl p-1 ">
            <div className="m-1 flex h-full w-2/5 flex-col rounded-xl bg-red-500">
              <div className="mt-1 flex h-3/6 w-full flex-col rounded-xl bg-blue-200 p-1">
                <div className="flex h-1/6 w-full">나의 에러 한눈에 보기</div>
                <div className="m-1 mb-3 flex h-5/6">
                  <div className="m-1 flex h-full w-1/4 flex-col rounded-xl bg-slate-100 p-2">
                    <img
                      className="h-12 w-12"
                      src="/icons/TempIcon.png"
                      alt="TempIcon"
                    />
                    <div className="mt-2 text-sm">해결 에러</div>
                    <div className="text-base">10</div>
                  </div>
                  <div className="m-1 flex h-full w-1/4 flex-col rounded-xl bg-slate-100 p-2">
                    <img
                      className="h-12 w-12"
                      src="/icons/TempIcon.png"
                      alt="TempIcon"
                    />
                    <div className="mt-2 text-sm">미해결 에러</div>
                    <div className="text-base">10</div>
                  </div>
                  <div className="m-1 flex h-full w-1/4 flex-col rounded-xl bg-slate-100 p-2">
                    <img
                      className="h-12 w-12"
                      src="/icons/TempIcon.png"
                      alt="TempIcon"
                    />
                    <div className="mt-2 text-sm">작성한 노트수</div>
                    <div className="text-base">10</div>
                  </div>
                  <div className="m-1 flex h-full w-1/4 flex-col rounded-xl bg-slate-100 p-2">
                    <img
                      className="h-12 w-12"
                      src="/icons/TempIcon.png"
                      alt="TempIcon"
                    />
                    <div className="mt-2 text-sm">최근 에러 발생일</div>
                    <div className="text-base">10</div>
                  </div>
                </div>
              </div>
              <div className="mt-1 flex h-3/6 w-full flex-col rounded-xl bg-green-300 p-1">
                <div className="flex h-1/6 w-full">나의 노트 한눈에 보기</div>
                <div className="m-1 mb-3 flex h-5/6">
                  <div className="m-1 flex h-full w-1/4 flex-col rounded-xl bg-slate-100 p-2">
                    <img
                      className="h-12 w-12"
                      src="/icons/TempIcon.png"
                      alt="TempIcon"
                    />
                    <div className="mt-2 text-sm">에러 노트</div>
                    <div className="text-base">10</div>
                  </div>
                  <div className="m-1 flex h-full w-1/4 flex-col rounded-xl bg-slate-100 p-2">
                    <img
                      className="h-12 w-12"
                      src="/icons/TempIcon.png"
                      alt="TempIcon"
                    />
                    <div className="mt-2 text-sm">기본 노트</div>
                    <div className="text-base">10</div>
                  </div>
                  <div className="m-1 flex h-full w-1/4 flex-col rounded-xl bg-slate-100 p-2">
                    <img
                      className="h-12 w-12"
                      src="/icons/TempIcon.png"
                      alt="TempIcon"
                    />
                    <div className="mt-2 text-sm">하암..</div>
                    <div className="text-base">10</div>
                  </div>
                </div>
              </div>
            </div>
            <div className="m-1 flex h-full w-3/5 flex-col rounded-xl bg-red-200">
              <div className="m-2 h-[15%] w-full">로그 트리</div>
              <div className="flex h-[85%] w-full p-2">
                <img
                  className="h-full w-full rounded-xl bg-purple-200"
                  src="/LogTemp.png"
                  alt="LogTemp"
                />
              </div>
            </div>
          </div>

          <div className="flex h-[45%] w-full rounded-xl p-2">
            <div className="box-border flex h-full w-full flex-col rounded-xl bg-yellow-300 p-3">
              <div className="h-1/6 w-full">에러 리스트</div>
              <div className="h-5/6 w-full">
                <ErrorList />
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default DashBoardPage;
